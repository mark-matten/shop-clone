...
export function TryOnModal({ isOpen, onClose, clerkId }: TryOnModalProps) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (result.imageUrl) {
        ...
        // Track the outfit ID so we can update it when saving (instead of creating duplicate)
        if (result.outfitId) {
          setSavedOutfitId(result.outfitId);
        }
      }
    } catch (error) {
  ...
  const handleConfirmSave = async () => {
    ...
    try {
      ...
      // If we have an existing outfit (either from history or just generated), update it
      ...
      // For new outfits, upload and save
